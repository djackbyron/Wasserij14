...
class LaundryReporting(models.Model):
    _name = "em.laundry.mgt.reporting"
    _description = "Laundry Wash Analysis"
    _auto = False
    _order = 'date desc'
    ...
    state = fields.Selection([
        ('draft', 'Draft Quotation'), ('order', 'Laundry Order'),
        ('process', 'Processing'),
        ('sent', 'Quotation Sent'),
        ('sale', 'Sales Order'),
        ('done', 'Sales Done'),
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause=''):
        ...
        select_ = """
            ...
            s.state as state,
            s.is_laundry_order as is_laundry_order,
        ...
        return '%s (SELECT %s FROM %s WHERE l.product_id IS NOT NULL AND s.is_laundry_order = True GROUP BY %s)' % (
